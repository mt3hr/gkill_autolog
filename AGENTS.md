...
| `src/autolog/internal/gkillclient/**` | [autolog-gkill-api](.claude/skills/autolog-gkill-api/SKILL.md) | HTTP 200 の本文 `errors` を見ずに成功扱いし、書けていない分を台帳へ載せて**二度とやり直されなくなる**／`add_urlog` を1秒1件へ抑えないと、サーバ側が1件ごとに対象URLを取りに行くので大量の外向きフェッチが出る／提案ごとに再ログインすると IP ごと15分10回の枠を使い潰し、その端末の取り込みが全滅する／`update_user_reps` を手で叩くと全件置換で既存 rep が消える |
