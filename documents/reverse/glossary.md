...
**記録した事実のうち何を gkill へ出さないか、利用者が調整できるのはこの2枚だけ**です。
判断を人にも AI にも都度求めません。要件で決まっている固定の取捨（ブラウザ内部ページや
YouTube 系の閲覧の除外、常駐通知の除外、30秒・1分などの閾値）は normalize のコードにあります。

**何を観測するか自体の取捨は別**です。Android は設定画面のチェックボックスで、
アプリ利用・通知・再生・Chrome履歴・Wi-Fi・Bluetooth・充電・スクリーンショット・
位置情報・音声を種類ごとに選べます。端末の利用（ロック解除・画面消灯）だけは
選べません。normalize が利用セッションの区間を組み立てる土台のためです。
...
| 通知 | Kmemo（テキスト） |
| スクリーンショット | IDF（ファイル） |
| 定期録音（Android） | IDF（ファイル。gkill が音声として扱い再生できる） |
