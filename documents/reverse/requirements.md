...
2. Accessibility Serviceから画面表示情報を取得
3. root権限でアプリ内部の履歴などを確認
4. URLまたは動画IDを直接確認できない場合は生ログだけ残す

URLを取得できない場合、検索URLや推測したURLは生成しない。Kmemoへの代替記録も行わない。
