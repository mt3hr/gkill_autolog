...
sequenceDiagram
    ...
    I->>R: カーソル〜上限時刻を読む（低水位マークがあればそこまで遡る）
    ...
    I->>R: 持ち越し（開区間・通知の既読）を保存
    I->>R: 処理カーソルを進める（読んだ後の挿入を検査し、低水位マークを更新・削除）
```
...
sequenceDiagram
    ...
    A->>S: events/<端末>-<時刻>-<連番>.jsonl.tmp へ書く
    A->>S: .jsonl へ名前を変える
...
sequenceDiagram
    ...
    Note over Y: 同期のたび
    Y->>V: dvnf move <置き場>/*.webp AutoScreenshot
...
撮り逃した最後の1区切りを次に画面が点いた時点で撮ります（アプリの設定で
無効にできます。既定は有効）。
このとき記録するのは**実際に撮れた時刻**で、区切りの時刻ではありません。
`*.webp` だけを渡すのは、書きかけの `.tmp` を運ばないためです。
...
sequenceDiagram
    ...
    I->>R: カーソルを提案B の時刻へ (AdvanceCursorChecked)

    Note over I: 次回の実行
    I->>R: 提案B の手前から読む
    I->>I: 提案A・C は台帳にあるので除外
    Note over I: 読み直しで別 ID の断片ができても、元イベントの包含 (CoveredCount) で除外。<br>一部だけ書き込み済みの区間系は Overlapped として破棄
    I->>G: 提案B だけ書き込む
```
